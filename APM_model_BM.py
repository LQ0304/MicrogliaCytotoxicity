...
for res in res_list:
    ...
    s = repre_list[res]
    load_data_path = os.path.join('./data/save_data_BM/', s)
    ...
    for i in range(0, num_data):
        ...
        save_model_path = os.path.join('./data/BM_RF_Results/', s)
        os.makedirs(save_model_path, exist_ok=True)

        save_data_path = './data/BM_RF_Results/'
        ...
        pre_train_y = RF_model.predict(train_x)
        pre_test_y = RF_model.predict(test_x)
        joblib.dump(RF_model, os.path.join(save_model_path, 'BM_RF_' + s + str(i) + '.pkl'))

        # '''SVM_classifier_Model'''
        # save_data_path = './data/BM_SVM_Results/'
        # os.makedirs(save_data_path, exist_ok=True)
        #
        # save_model_path = os.path.join('./data/BM_SVM_Results/', s)
        # os.makedirs(save_model_path, exist_ok=True)
        #
        ...
        # pre_train_y = SVM_Model.predict(train_x)
        # pre_test_y = SVM_Model.predict(test_x)
        # joblib.dump(SVM_Model, os.path.join(save_model_path, 'BM_SVM_' + s + str(i) + '.pkl'))

        # # '''GBDT model'''
        # save_data_path = './data/BM_GBDT_Results/'
        # os.makedirs(save_data_path, exist_ok=True)
        #
        # save_model_path = os.path.join('./data/BM_GBDT_Results/', s)
        # os.makedirs(save_model_path, exist_ok=True)
        #
        ...
        #     global train_x, train_y
        #
        #     GBDT_model = GradientBoostingClassifier(random_state=22,
        ...
        # pre_train_y = GBDT_model.predict(train_x)
        # pre_test_y = GBDT_model.predict(test_x)
        # joblib.dump(GBDT_model, os.path.join(save_model_path, 'BM_GBDT_' + s + str(i) + '.pkl'))
