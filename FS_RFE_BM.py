...
save_path = './data/save_data_BM_FS'
os.makedirs(save_path, exist_ok=True)
load_data_path = './data/save_data_BM/'
...
#         x_data_s = np.concatenate([train_x, test_x], axis=0)
#         y_data_s = np.concatenate([train_y, test_y], axis=0)
#         x_data_s_rezero, rezero_index = remove_zero_column(x_data_s)
#         s = s + '_RFE'
#         save_path_s = os.path.join(save_path, s)
#         os.makedirs(save_path_s, exist_ok=True)
#         FS_index, s_score, s_alpha_coef, s_best_score, s_best_alpha = \
#             RFE_FS(FS_model, x_data_s_rezero, y_data_s, rezero_index,
#                    start, end, step_len, RFE_step, s, save_path_s, i)
#         x_data_s_FS = x_data_s[:, FS_index]
#         np.save(os.path.join(save_path_s, s + '_' + 'index%d.npy' % i), FS_index)
#         save(save_path_s, s, x_data_s_FS, y_data_s, len(train_y), len(y_data_s), i)
#         print('%s contain the number of features:%d' % (s, len(x_data_s_FS[1])))
