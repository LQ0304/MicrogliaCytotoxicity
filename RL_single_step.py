...
with open('./data/smiles_name101', 'r') as f:
    data_line = f.readlines()

parent_compound_name_list = []
parent_smiles_list = []
for data in data_line:
    smiles, compound_name, label = data.strip('\n').split('\t')
    if int(label) == 0:
        parent_smiles_list.append(smiles)
        parent_compound_name_list.append(compound_name)
print('Cytotoxic contain:%d' % (len(parent_smiles_list)))
...
for i in range(len(parent_smiles_list)):
    parent_compound_name = parent_compound_name_list[i]
    parent_smiles = parent_smiles_list[i]
    parent_mol = Chem.MolFromSmiles(parent_smiles)
    parent_fp, _ = mol_to_ecfp4(parent_mol, 2048)  # 训练集的BM，用于计算骨架相似度
    ...
    for info_key, info_value in bitVec_info.items():
        if info_key == feature:
            if len(info_value) == 1:
                save_path = './data/One_step/Generate_molecule_%s/' % parent_compound_name
                ...
                if parent_mol.HasSubstructMatch(submol):
                    ...
                    Flag_list = []
                    BM_sim_list = []
                    for j in range(len(states_smiles)):
                        smiles = states_smiles[j]
                        mol = states_mol[j]
                        act = actions[j]
                        ...
                        hit_atom_t1_list = atom_list[j]
                        hit_bond_t1_list = bond_list[j]
                        ...
                        img.save(os.path.join(save_path, str(j) + '.png'), dpi=(1000, 1000))
                        img.close()
                    print('%s_States contain:%d' % (parent_compound_name, len(states_smiles)))
                    ...
                    df.to_excel(os.path.join(save_path, '%s_generate_smiles.xlsx' % parent_compound_name), index=False)
            else:
                save_path = './data/Two_step/Generate_molecule_%s/' % parent_compound_name
                os.makedirs(save_path, exist_ok=True)
