...
def add_predict_args(parser: ArgumentParser):
    ...
    args.gpu = 0
    args._test_path = './dataset/testset.csv'
    args._features_path = './dataset/testset-feat.npy'
    args._preds_path = 'preds_covidvs1_testset.csv'
    args._checkpoint_dir = 'covidvs1/'
    args._use_compound_names = True
    default='./data/finetunev2-feat.npy',
    """
    ...
    parser.add_argument('--features_path', type=str, nargs='*',
                        help='Which GPU to use')
    parser.add_argument('--test_path', type=str, default='./data/BV-2 cell toxicity.csv',
                        help='Path to CSV file containing testing data for which predictions will be made')
    parser.add_argument('--use_compound_names', action='store_true', default=False,
                        help='Use when test data file contains compound names in addition to SMILES strings')
    parser.add_argument('--preds_path', type=str, default='preds_covidvs1_testdata.csv',
                        help='Path to CSV file where predictions will be saved')
    parser.add_argument('--checkpoint_dir', type=str, default='model/mpnn/',
...
def add_train_args(parser: ArgumentParser):
    ...
    parser.add_argument('--hidden_size', type=int, default=2300,
...
def modify_train_args(args: Namespace):
    ...
    if args.save_dir is not None:  # save_dir为none
        makedirs(args.save_dir)    # 创建一个给定目录或文件路径的目录。
    else:
        temp_dir = TemporaryDirectory()  # 创建并返回一个临时目录
    ...
    del args.no_features_scaling  # 删除

    if args.metric is None:   # 上文args.metric为none
    ...
    args.minimize_score = args.metric in ['rmse', 'mae', 'mse', 'cross_entropy']  # 布尔值
    ...
    if args.features_generator is not None and 'rdkit_2d_normalized' in args.features_generator:  # features_generator空
        assert not args.features_scaling

    args.num_lrs = 1

    if args.ffn_hidden_size is None:   # ffn_hidden_size为 none hidden_size为 300
    ...
    if args.split_type in ['crossval', 'index_predetermined']:       # 划分种类问题 为random
