...
class Vocabulary(object):
    ...
    def __init__(self, init_from_file=None, max_length=120):
...
class MolData(Dataset):
    ...
    def __getitem__(self, i):
        ...
        tokenized = self.voc.tokenize(mol)    # 将一个SMILES的字母分开
        encoded = self.voc.encode(tokenized)  # 将一个SMILES的字母码变为索引码
        # return Variable(encoded), label
    ...
    def collate_fn(cls, arr):
        ...
        # 定义如何选取样本的，归整化一个Batch_size
        # max_length = max([seq[0].size(0) for seq in arr])              # Batch_size中每个SMILES码的最大长度
        max_length = max([seq.size(0) for seq in arr])              # Batch_size中每个SMILES码的最大长度
        # labels_data = [seq[1] for seq in arr]
        collated_arr = Variable(torch.zeros(len(arr), max_length))     # 创建一个全0的Variable，(batch_size,max_length)
        for i, seq in enumerate(arr):
            collated_arr[i, :seq.size(0)] = seq                  # 将arr中内容写入到collated_arr
            # collated_arr[i, :seq[0].size(0)] = seq[0]                  # 将arr中内容写入到collated_arr
        # return collated_arr, labels_data
        return collated_arr

class MolData_agent(Dataset):
    """Custom PyTorch Dataset that takes a file containing SMILES.
        Args:
                fname : path to a file containing \n separated SMILES.
                voc   : a Vocabulary instance
    ...
    def __init__(self, fname, voc):
        ...
        with open(fname, 'r') as f:
            ...
            for line in lines:
                self.smiles.append(line.split()[0])
                self.labels.append(line.split()[1])
                self.props.append(line.split()[2])


    def __getitem__(self, i):
        mol = self.smiles[i]
        label = float(self.labels[i])
        prob = float(self.props[i])
        tokenized = self.voc.tokenize(mol)    # 将一个SMILES的字母分开
        encoded = self.voc.encode(tokenized)  # 将一个SMILES的字母码变为索引码
        return Variable(encoded), label, prob
        # return Variable(encoded)
    ...
    def collate_fn(cls, arr):
        ...
        max_length = max([seq[0].size(0) for seq in arr])              # Batch_size中每个SMILES码的最大长度
        labels_data = [seq[1] for seq in arr]
        prop_data = [seq[2] for seq in arr]
        collated_arr = Variable(torch.zeros(len(arr), max_length))     # 创建一个全0的Variable，(batch_size,max_length)
        for i, seq in enumerate(arr):
            collated_arr[i, :seq[0].size(0)] = seq[0]                  # 将arr中内容写入到collated_arr
        return collated_arr, labels_data, prop_data

class MolData_qed_sa(Dataset):
    """Custom PyTorch Dataset that takes a file containing SMILES.
        Args:
                fname : path to a file containing \n separated SMILES.
                voc   : a Vocabulary instance
