...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        # 注意num为窗口大小
        num = int(bool(config.num_props)) +  int(config.scaffold_maxlen) # 骨架和性质属性
        # num = int(config.scaffold) # 骨架属性
        # num = int(bool(config.num_props)) # 性质属性
        # num = 1 # 无条件属性
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None, prop = None, scaffold = None):
        # idx,targets (B,T) prop(B,1) scaffold(B,Ts)
        b, t = idx.size()
        ...
        token_embeddings = self.tok_emb(idx) #(B,T,H) # each index maps to a (learnable) vector
        position_embeddings = self.pos_emb[:, :t, :] #(1,T,B)# each position maps to a (learnable) vector
        type_embeddings = self.type_emb(torch.ones((b,t), dtype = torch.long, device = idx.device)) #(B,T,H)
        ...
        if self.config.num_props:
            ...
            # p = self.prop_nn(prop.unsqueeze(1)) #(B,1,H)单一属性
            p = self.prop_nn(prop) # 多属性
            p += type_embd
            x = torch.cat([p, x], 1)
