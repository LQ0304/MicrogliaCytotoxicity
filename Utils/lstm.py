...
class Generator(nn.Module):
    ...
    def __init__(self, voc, voc_size, num_layers, embed_dim, hidden_dim):
        ...
        self.lstm = nn.LSTM(input_size=embed_dim,
                            ...
                            dropout=0,
                            bidirectional=False)
        self.lstm2out = nn.Linear(hidden_dim, voc_size)
    ...
    def forward(self, x, init_state):  # x 128 'GO',
        ...
        out, h = self.lstm(x, init_state)       # out 128*1*512 h 2 3*128*512
        out = self.lstm2out(out.squeeze(1))     # [128,62]
        return out, h
    ...
    def likelihood(self, inputs):
        ...
        for step in range(seq_length):                             # for循环的作用是从一个SMILES码开始取元素所对应的索引值，以列学习每个词
                                                                   # 学习每一个SMILES的encode的前一个元素与后一个元素的关系
    ...
    def sample(self, batch_size, max_length=100, x=None):
        """
        ...
        if flag:
            for step in range(max_length):  # 遍历step（140）
                ...
                prob = F.softmax(logits, dim=1)  # prob:128*62        # F.log_softmax是Pytorch中定义的函数，输出为log_prob
            ...
            given_len = x.size(1)
            lis = x.chunk(x.size(1), dim=1)  # chunk方法可以对张量分块，返回一个张量列表
            for i in range(given_len):
                logits, h = self.forward(lis[i].squeeze(1), h)  # lis[i]:128*1
                prob = F.softmax(logits, dim=1)
                sequences.append(lis[i])
            x = prob.multinomial(1)
            for i in range(given_len, max_length):
                sequences.append(x)
                logits, h = self.forward(x.squeeze(1), h)
                prob = F.softmax(logits, dim=1)
                x = prob.multinomial(1)
