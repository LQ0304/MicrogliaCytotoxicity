'''
function:6种分子表征形式（Brics、ECFP、FCFP、Rdkit、MACCS、Avalon）
无特征选择，对应的fp_bit_list 和label_list
'''
...
save_path = './data/save_data_BM/'
os.makedirs(save_path, exist_ok=True)
dataset_path = './data/BM_Train_Test_dataset/'

# load data
for rank in range(0, 10):
    with open(os.path.join(dataset_path, 'BM_train_dataset%d.txt' % (rank)), 'r') as f:
        data_line1 = f.readlines()
    print('train data contain:%d' % (len(data_line1)))

    # load data
    with open(os.path.join(dataset_path, 'BM_test_dataset%d.txt' % (rank)), 'r') as f:
    ...
    for data in data_line:
        smiles, mol_name, label = data.strip('\n').split('\t')
    ...
    save_path_maccs = os.path.join(save_path, s)
    os.makedirs(save_path_maccs, exist_ok=True)
    save(save_path_maccs, s, x_data_maccs, y_data_maccs,
         len(data_line1), len(data_line), rank)
    ...
    s = repre_list[6]
    train_data_path = './data/BM_Train_Test_dataset_CSV/BM_train_dataset%d' % rank
    ...
    test_data_path = 'data/BM_Train_Test_dataset_CSV/BM_test_dataset%d' % rank
